...
export default function SignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const { data, error: authErr } = await authClient.signUp.email({ name, email, password })
      if (authErr) { setError(authErr.message || "Registration failed"); return }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#06060a] p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Join SelfTracker</p>
        </div>
        <form onSubmit={handleSubmit} className="card space-y-4">
          {error && <p className="text-sm text-brand-red bg-brand-red/10 p-3 rounded-lg">{error}</p>}
          <div>
            <label className="text-xs text-gray-500 font-medium uppercase tracking-wider">Name</label>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)}
              className="w-full mt-1 bg-bg-hover border border-bg-border rounded-xl px-4 py-3 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-brand-blue/50" placeholder="Ahmed Lotfy" required />
          </div>
          <div>
          ...
          <p className="text-xs text-gray-600 text-center">
            ...
            <button type="button" onClick={() => navigate("/sign-in")} className="text-brand-blue hover:underline">Sign in</button>
          </p>
        </form>
      </div>
    </div>
