...
USDA_NUTRIENT_MAP = {
    ...
    "1257": "trans_fat_100g", "1258": "saturated_fat", "2000": "sugar",
}
...
def make_usda_row(f, n, source):
    return "\t".join([
        ...
        val(n.get("vitamine_b9_100g") if False else n.get("vitamin_b9_100g")),
...
def make_branded_row(f, n):
    s = str(f.get("serving_size", ""))
    try:
        sv = str(float(s)) if s and float(s) == float(s) else "\\N"
    except:
        sv = "\\N"
    return "\t".join([
        val(f.get("fdc_id")),
...
def import_usda(db_url, source, food_file, nutrient_file, limit):
    ...
    for i in range(max_rows):
        ...
        lines.append(make_usda_row(f, n, source))
...
def import_branded(db_url, branded_food_file, nutrient_file, limit):
    ...
    for i in range(max_rows):
        f = foods[i]
        n = nmap.get(f.get("fdc_id", ""), {})
        lines.append(make_branded_row(f, n))
...
def main():
    ...
    # Get DB URL from container's .env
    db_url = None
    env_paths = ["/app/.env", "/tmp/food-imports/.env", os.path.expanduser("~/.env")]
    ...
    if not db_url:
        print("ERROR: DATABASE_URL not found. Set it as env var or in .env file")
    ...
    print(f"[1/4] Truncated. Foods: {cur.fetchone()[0]}")
    conn.close()
