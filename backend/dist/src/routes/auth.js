...
import { findUserByEmail, findUserById, generateTokens, generateVerificationToken, } from "../../lib/utility.js";
...
authRouter.post("/register", async (c) => {
    ...
    try {
        ...
        return c.json({ message: err instanceof Error ? err.message : "Something went wrong!" }, 500);
    }
});
authRouter.get("/verify-email", async (c) => {
    const token = c.req.query("token");
    if (!token)
        return c.json({ message: "Invalid verification link!" }, 400);
    try {
        ...
        await db
            .update(users)
            .set({ isVerified: true })
            .where(eq(users.id, decoded.payload.userId));
        await db
