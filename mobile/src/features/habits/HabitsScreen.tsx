  ...
  return (
    <View className="flex-1 bg-background px-2">
      <View className="flex-1">
        ...
        {/* Progress Card */}
        <View className="bg-card border border-border p-5 rounded-2xl mb-6 shadow-sm mt-4">
          <View className="flex-row justify-between items-center mb-4">
            <View>
              <Text className="text-xs font-bold text-placeholder uppercase tracking-widest mb-1">Daily Progress</Text>
              <View className="flex-row items-center">
                <Ionicons name="trophy" size={20} color="#EAB308" style={{ marginRight: 8 }} />
                <Text className="text-3xl font-extrabold text-primary">{completionRate}%</Text>
              </View>
            </View>
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
          {habits.length === 0 ? (
            <View className="items-center justify-center py-10 border-2 border-dashed border-border rounded-2xl bg-card/50">
              <View className="bg-muted p-4 rounded-full mb-4">
                <Ionicons name="trophy-outline" size={32} color={colors.placeholder} />
              </View>
              <Text className="text-lg font-semibold text-text mb-1">No habits yet</Text>
              <Text className="text-placeholder text-center px-8">Create your first habit to start tracking.</Text>
            </View>
          ) : (
              ...
              {habits.map((habit: any) => (
                ...
                >
                  <View className="flex-1 mr-4">
                    <Text className={`text-lg font-bold mb-1 ${habit.completedToday ? 'text-primary' : 'text-text'}`}>
                      {habit.name}
                    </Text>
                    <View className="flex-row items-center">
                      <View className={`flex-row items-center px-2 py-0.5 rounded-full mr-2 ${habit.streak > 0 ? 'bg-orange-100 dark:bg-orange-900/30' : 'bg-muted'
                        }`}>
                        <FontAwesome5 name="fire" size={10} color={habit.streak > 0 ? "#F97316" : colors.placeholder} style={{ marginRight: 4 }} />
                        <Text className={`text-xs font-bold ${habit.streak > 0 ? 'text-orange-500' : 'text-placeholder'}`}>
                  ...
                  <View className={`h-8 w-8 rounded-full items-center justify-center border-2 ${habit.completedToday ? 'bg-primary border-primary' : 'border-placeholder bg-transparent'
                    }`}>
        ...
        >
          <View className="flex-1 justify-end bg-black/50">
            <View className="bg-card rounded-t-3xl p-6 h-[40%]">
              <View className="flex-row justify-between items-center mb-6">
                <Text className="text-xl font-bold text-text font-orbitron">Create New Habit</Text>
              ...
              >
                <Text className={`font-bold text-lg ${!newHabitName.trim() ? 'text-placeholder' : 'text-white'
                  }`}>
                  Create Habit
                </Text>
              </Pressable>
