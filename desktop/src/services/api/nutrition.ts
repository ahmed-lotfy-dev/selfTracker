...
export async function analyzeFoodImage(base64Image: string): Promise<FoodAnalysisResult> {
  const res = await axiosInstance.post<FoodAnalysisResult>("/api/nutrition/analyze", { image: base64Image })
  return res.data
}
...
export async function updateNutritionGoals(data: NutritionGoals): Promise<void> {
