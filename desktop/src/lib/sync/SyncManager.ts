import { TauriSQLiteAdapter, db as adapterInstance, ElectricSync } from "@/db/client"
...
class SyncManagerService {
  ...
  async initialize() {
    ...
    try {
      ...
      // Use the exported instance which wraps the plugin-sql logic
  ...
  async pullFromDB() {
    ...
    try {
      ...
      // Always update store if we have data, or if empty (to consistent state)
      ...
      if (habits.length > 0 || true) useHabitsStore.getState().setHabits(habits)
      ...
      if (workouts.length > 0 || true) useWorkoutsStore.getState().setWorkouts(workouts)
      ...
      if (workoutLogsResult.length > 0 || true) useWorkoutsStore.getState().setWorkoutLogs(workoutLogs)
      ...
      if (weightsResult.length > 0 || true) useWeightStore.getState().setWeightLogs(weightLogs)
  ...
  async pushHabit(habit: any) {
    if (!this.db) return
    console.log('[SyncManager] Pushing habit:', JSON.stringify(habit))
    try {
      // Local Write
      await this.db.runAsync(`
